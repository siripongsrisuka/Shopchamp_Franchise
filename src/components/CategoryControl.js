...
import { checkCategory2, manageCategory } from "../Utility/function";

const { white, redPink } = colors;
            ...
            {warehouseCategory.map((item,index)=>{
                ...
                <div key={index} style={styles.container3} className="custom-scrollbar" >
                    ...
                    {item.level === 1 ? (
                        ...
                        className="category"
                        style={{
                            backgroundColor: length ? redPink : null,
                    ...
                    {checkCategory2(categorySetting, item).map((a, i) => {
                        const status = categorySetting.some(b=>b.id === a.id);
                        return (
                        <div
                            onClick={() => {setCategorySetting(manageCategory(item.value, categorySetting, a))}}
                            key={i}
                            className="category"
                            style={{
                                backgroundColor: status ? redPink : null,
